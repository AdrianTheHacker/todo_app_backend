...
fn add_event(user: &str, event: &str) {
    ...
    fs::write(file_path, json).expect("Unable to write file");      // Writes new JSON to the users JSON file.

    println!("Successfully added '{}' to {}'s todo list", event, user);
}

// functions to implement
fn delete_event(user: &str, event: &str) {}
